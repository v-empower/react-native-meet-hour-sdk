...
module.exports = {
  resolver: {
    blacklistRE: blacklist([
      /ios\/Pods\/MeetHourSDK\/Frameworks\/MeetHour.framework\/assets\/node_modules\/react-native\/.*/,
    ]),
  },
};
```
...
class VideoCall extends React.Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      const url = 'https://meethour.io/roomName'; // can also be only room name and will connect to meet hour servers
      const userInfo = { displayName: 'User', email: 'user@example.com', avatar: 'https:/gravatar.com/avatar/abc123' };
      MeetHour.call(url, userInfo);
      /* You can also use MeetHour.audioCall(url) for audio only call */
      /* You can programmatically end the call with MeetHour.endCall() */
    }, 1000);
  }
  ...
  render() {
    return (
      <View style={{ backgroundColor: 'black',flex: 1 }}>
        <MeetHourView onConferenceTerminated={this.onConferenceTerminated} onConferenceJoined={this.onConferenceJoined} onConferenceWillJoin={this.onConferenceWillJoin} style={{ flex: 1, height: '100%', width: '100%' }} />
      </View>
    );
  }
}
...
1.) import MeetHour and MeetHourEvents: `import MeetHour, { MeetHourEvents } from 'react-native-meet-hour-sdk';`
...
```
  const initiateVideoCall = () => {
    MeetHour.initialize();
    MeetHourEvents.addListener('CONFERENCE_LEFT', (data) => {
      console.log('CONFERENCE_LEFT');
    });
    setTimeout(() => {
      MeetHour.call(`<your url>`);
    }, 1000);
  };
```
### Events
...
- 1-2.) choose `node_modules/react-native-meet-hour-sdk/ios/RNMeetHour.xcodeproj` then `Add`  
- 1-3.) add `node_modules/react-native-meet-hour-sdk/ios/WebRTC.xcframework` and `node_modules/react-native-meet-hour-sdk/ios/MeetHour.framework` to the Frameworks folder
- 1-4.) add `node_modules/react-native-meet-hour-sdk/ios/MeetHour.storyboard` in the same folder as AppDelegate.m
...
This will create a navigation controller to be able to navigate between the Meet Hour component and your react native screens.
...
2-3.) add path on BOTH sections with: `$(SRCROOT)/../node_modules/react-native-meet-hour-sdk/ios` with `recursive`  

### Step 3. Change General Setting and Embed Framework

3-1.) go to `General` tab  
3-2.) change `Deployment Target` to `8.0`  
3-3.) add `WebRTC.xcframework` and `MeetHour.framework` in `Embedded Binaries` 

### Step 4. Link/Include Necessary Libraries

- 4-1.) click `Build Phases` tab, open `Link Binary With Libraries`  
- 4-2.) add `libRNMeetHour.a`  
- 4-3.) make sure `WebRTC.xcframework` and `MeetHour.framework` linked  
...
<permission android:name="${applicationId}.permission.MEETHOUR_BROADCAST"
    android:label="Meet Hour Event Broadcast"
    android:protectionLevel="normal"></permission>
<uses-permission android:name="${applicationId}.permission.MEETHOUR_BROADCAST"/>
```

2.) In the `<application>` section of `android/app/src/main/AndroidManifest.xml`, add
 ```xml
 <activity android:name="go.meethour.io.react.sdk.MeetHourNavigatorActivity" />
 ```
 
3.) In `android/settings.gradle`, include react-native-meet-hour-sdk module
```gradle
include ':react-native-meet-hour-sdk'
project(':react-native-meet-hour-sdk').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-meet-hour-sdk/android')
```

4.) In `android/app/build.gradle`, add react-native-meet-hour-sdk to dependencies
```gradle
android {
  ...
  packagingOptions {
...
```java
import go.meethour.io.react.sdk.RNMeetHourPackage;  // <--- Add this line
...
...
    ...
    protected List<ReactPackage> getPackages() {
      return Arrays.<ReactPackage>asList(
        ...
        new RNMeetHourPackage()                  // <--- Add this line
      );
    }
```
...
If your app already includes `react-native-locale-detector` or `react-native-vector-icons`, you must exclude them from the `react-native-meet-hour-sdk` project implementation with the following code (even if you're app uses autolinking with RN > 0.60):
